...
public class EnemyEvilBat : EnemyBase
{
    ...
    [SerializeField] private float patrolRadius = 5f; // Radio desde la posici�n inicial
    [SerializeField] private float waypointReachDistance = 0.2f; // Distancia para considerar que lleg� al punto
    [SerializeField] private bool useStartPositionAsCenter = true; // Usar posici�n inicial como centro
    ...
    [SerializeField] private float pathValidationRayCount = 8; // N�mero de rayos para validar el camino
    [SerializeField] private LayerMask pathObstacleLayer; // Layer de obst�culos para pathfinding

    [Header("Wing Flap Movement Settings")]
    [SerializeField] private float flapAmplitude = 0.3f; // Amplitud del aleteo (qu� tan alto/bajo)
    [SerializeField] private float flapFrequency = 3f; // Frecuencia del aleteo (qu� tan r�pido)
    [SerializeField] private float horizontalWaveAmplitude = 0.2f; // Ondulaci�n horizontal
    [SerializeField] private float horizontalWaveFrequency = 2f;
    [SerializeField] private bool usePerlinNoise = true; // Usar ruido Perlin para movimiento m�s org�nico
    ...
    [SerializeField] private float deathFallSpeed = 20f; // Velocidad de ca�da adicional
    ...
    private void FindPlayer()
    {
        if (player == null)
        {
            ...
            if (playerObj != null)
            {
                ...
                if (logBehaviorDetails)
                {
                    Debug.Log($"{gameObject.name} encontr� al jugador: {player.name}");
                }
            }
        }
    }
    ...
    private void UpdateLineOfSight()
    {
        ...
        if (visionCheckTimer <= 0f)
        {
            ...
            if (previousLineOfSight && !hasLineOfSight)
            {
                ...
                {
                    Debug.Log($"{gameObject.name} perdi� l�nea de visi�n temporalmente");
                }
            }
            else if (!previousLineOfSight && hasLineOfSight)
            {
                ...
                if (logBehaviorDetails && currentState == BatState.Chase)
                {
                    Debug.Log($"{gameObject.name} recuper� l�nea de visi�n");
                }
            }
        }
    ...
    private Vector2 CalculateFlapOffset(Vector2 baseDirection)
    {
        ...
        if (usePerlinNoise)
        {
            // Usar ruido Perlin para movimiento m�s org�nico y natural
        ...
        {
            // Usar senos para movimiento m�s predecible
        ...
        // Calcular direcci�n perpendicular para el movimiento horizontal
    ...
    private void ChangeState(BatState newState)
    {
        ...
        if (logBehaviorDetails)
        {
            Debug.Log($"{gameObject.name} cambi� de {currentState} a {newState}");
        }
        ...
        switch (newState)
        {
            case BatState.Waiting:
                ...
                waitingStartPosition = transform.position; // Guardar posici�n actual
                // Resetear los timers para un hover m�s suave
                flapTimer = 0f;
    ...
    private bool IsPathClear(Vector2 from, Vector2 to)
    {
        ...
        // Si la distancia es muy corta, siempre es v�lido
        if (distance < 0.5f)
        {
            return true;
        }
        ...
        // Validaci�n adicional con m�ltiples rayos para mejor detecci�n
        float angleStep = 360f / pathValidationRayCount;
        float checkRadius = 0.3f; // Radio de verificaci�n alrededor del punto
        ...
        for (int i = 0; i < pathValidationRayCount; i++)
        {
            ...
            if (hit.collider != null)
            {
                if (logBehaviorDetails)
                {
                    Debug.Log($"{gameObject.name}: Camino bloqueado en �ngulo {angle}� por {hit.collider.name}");
                }
                return false;
            }
        }
    ...
    private bool IsPointValid(Vector2 point)
    {
        ...
        if (hit != null)
        {
            if (logBehaviorDetails)
            {
                Debug.Log($"{gameObject.name}: Punto {point} inv�lido, overlapping con {hit.name}");
            }
            return false;
        }
    ...
    private void GenerateNewPatrolPoint()
    {
        ...
        while (attempts < maxPathAttempts && !validPointFound)
        {
            ...
            if (validatePatrolPath)
            {
                // Primero verificar si el punto no est� dentro de un obst�culo
                if (!IsPointValid(candidatePoint))
                {
                    if (logBehaviorDetails)
                    {
                        Debug.Log($"{gameObject.name}: Intento {attempts} - Punto dentro de obst�culo");
                    }
                    continue;
                }
            ...
            // Punto v�lido encontrado
            currentPatrolTarget = candidatePoint;
            validPointFound = true;

            if (logBehaviorDetails)
            {
                Debug.Log($"{gameObject.name}: Punto v�lido encontrado en intento {attempts}: {currentPatrolTarget}");
            }
        }

        // Si no se encontr� un punto v�lido despu�s de todos los intentos
        if (!validPointFound)
        {
            // Fallback: usar la posici�n actual como objetivo (quedarse quieto)
            currentPatrolTarget = transform.position;

            if (logBehaviorDetails)
            {
                Debug.LogWarning($"{gameObject.name}: No se encontr� punto v�lido despu�s de {maxPathAttempts} intentos. Qued�ndose en posici�n actual.");
            }
        }
    }
    ...
    private void WaitBehavior()
    {
        ...
        if (rb != null)
        {
            // Movimiento suave hacia la posici�n de hover
    ...
    private void CheckForPlayer()
    {
        ...
        if (distanceToPlayer <= detectionRange && hasLineOfSight)
        {
            if (logBehaviorDetails)
            {
                Debug.Log($"{gameObject.name} detect� al jugador a {distanceToPlayer:F2} unidades");
            }
    ...
    private void ChaseBehavior()
    {
        ...
        // Aplicar movimiento con aleteo (m�s agresivo en persecuci�n)
        ApplyFlapMovement(direction, speed);
    ...
    private void CheckLoseTarget()
    {
        ...
        if (tooFar || lostVisionTooLong)
        {
            if (logBehaviorDetails)
            {
                Debug.Log($"{gameObject.name} perdi� de vista al jugador");
            }
            timeWithoutLineOfSight = 0f;
            ChangeState(BatState.Patrol);
        }
    ...
    private void ExecuteAttack()
    {
        ...
        foreach (Collider2D hit in hits)
        {
            ...
            if (damageable != null)
            {
                ...
                Debug.Log($"{gameObject.name} golpe� a {hit.gameObject.name}");
            }
        }
    ...
    protected override void OnDamageTakenCustom(int damageAmount)
    {
        ...
        if (logBehaviorDetails)
        {
            Debug.Log($"{gameObject.name} recibi� {damageAmount} de da�o");
        }
    ...
    protected override void OnDeathCustom()
    {
        ...
        if (animationController != null)
        {
            ...
            if (logBehaviorDetails)
            {
                Debug.Log($"{gameObject.name}: Animaci�n de muerte activada");
            }
        }
        else
        {
            Debug.LogWarning($"{gameObject.name}: No se encontr� BatAnimationController!");
        }

        // HACER QUE CAIGA SUPER R�PIDO
        if (rb != null)
        {
            // Activar gravedad alta
            rb.gravityScale = deathGravityScale;

            // Aplicar velocidad de ca�da inmediata hacia abajo
            rb.linearVelocity = new Vector2(0f, -deathFallSpeed);

            // Asegurarse de que el Rigidbody siga activo para que la f�sica funcione
            rb.simulated = true;

            if (logBehaviorDetails)
            {
                Debug.Log($"{gameObject.name}: Ca�da r�pida activada (Gravedad: {deathGravityScale}, Velocidad: {deathFallSpeed})");
            }
        }
